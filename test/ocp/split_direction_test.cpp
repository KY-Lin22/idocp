...
TEST_F(SplitDirectionTest, fixed_base) {
  ...
  const Eigen::VectorXd dq = split_direction.segment(3*robot.dimv()+robot.dim_passive()+7*robot.num_point_contacts(), robot.dimv());
  const Eigen::VectorXd dv = split_direction.segment(4*robot.dimv()+robot.dim_passive()+7*robot.num_point_contacts(), robot.dimv());
  const Eigen::VectorXd dx = split_direction.segment(3*robot.dimv()+robot.dim_passive()+7*robot.num_point_contacts(), 2*robot.dimv());

...
TEST_F(RiccatiMatrixInverterTest, fixed_base_without_contacts) {
  ...
  ASSERT_EQ(dimfr, 0);
  ASSERT_EQ(dimc, 0);
  const Eigen::MatrixXd Qaa_seed = Eigen::MatrixXd::Random(dimafr, dimafr);
  const Eigen::MatrixXd Qaa = Qaa_seed * Qaa_seed.transpose() + Eigen::MatrixXd::Identity(dimafr, dimafr);
  const Eigen::MatrixXd C_afr = Eigen::MatrixXd::Random(dimc, dimafr);
  RiccatiMatrixInverter inverter(fixed_base_robot_);
  inverter.invert(Qaa, C_afr);
  Eigen::MatrixXd G_inv = Eigen::MatrixXd::Zero(dimafr, dimafr);
  inverter.getInverseMatrix(G_inv);
  Eigen::MatrixXd G_inv_ref = Qaa.inverse();
  EXPECT_TRUE(G_inv_ref.isApprox(G_inv, 1.0e-12));
  EXPECT_TRUE((G_inv*Qaa).isApprox(Eigen::MatrixXd::Identity(dimafr+dimc, dimafr+dimc), 1.0e-12));
  // const Eigen::MatrixXd dPvv = Eigen::MatrixXd::Random(dimv, dimv);
  // const double dtau = std::abs(Eigen::VectorXd::Random(1)[0]);
  // Eigen::MatrixXd dP_ref = Eigen::MatrixXd::Zero(dimafr+dimc, dimafr+dimc);
...
TEST_F(RiccatiMatrixInverterTest, fixed_base_with_contacts) {
  ...
  RiccatiMatrixInverter inverter(fixed_base_robot_);
  inverter.invert(Qaa, C_afr);
  Eigen::MatrixXd G_mat = Eigen::MatrixXd::Zero(dimafr+dimc, dimafr+dimc);
  G_mat.topLeftCorner(dimafr, dimafr) = Qaa;
  G_mat.topRightCorner(dimafr, dimc) = C_afr.transpose();
  G_mat.bottomLeftCorner(dimc, dimafr) = C_afr;
  std::cout << G_mat << std::endl;
  Eigen::MatrixXd G_inv_ref = G_mat.inverse();
  Eigen::MatrixXd G_inv = Eigen::MatrixXd::Zero(dimafr+dimc, dimafr+dimc);
  inverter.getInverseMatrix(G_inv);
  EXPECT_TRUE(G_inv_ref.isApprox(G_inv, 1.0e-12));
  EXPECT_TRUE((G_inv*G_mat).isApprox(Eigen::MatrixXd::Identity(dimafr+dimc, dimafr+dimc), 1.0e-12));
  // const Eigen::MatrixXd dPvv = Eigen::MatrixXd::Random(dimv, dimv);
  // const double dtau = std::abs(Eigen::VectorXd::Random(1)[0]);
  // Eigen::MatrixXd dP_ref = Eigen::MatrixXd::Zero(dimafr+dimc, dimafr+dimc);
  // // dP_ref.topLeftCorner(dimv, dimv) = dtau * dtau * dPvv;
